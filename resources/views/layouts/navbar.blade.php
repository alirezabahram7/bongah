...
@if(Auth::check())
        <a class="nav-link" href="{{route('profile.show',['id'=>auth()->user()->id])}}">
          @if(auth()->user()->photo!=null)
            <img title="{{auth()->user()->name}}" class="img-thumbnail rounded-circle img-responsive" style="width:50px" src="{{auth()->user()->photo}}">
          @else
            <img title="{{auth()->user()->name}}" class="img-thumbnail rounded-circle img-responsive" style="width:50px" src="/pic/nopro.png">
          @endif
          {{auth()->user()->name}}
        </a>
      @endif
    
    <button class="navbar-toggler" type="button" data-toggle="collapse" style="color:black;" data-target="#collapsibleNavbar">
    ...
    <div class="collapse navbar-collapse" id="collapsibleNavbar" >
      ...
      <ul class="container navbar-nav" >
        ...
        @if(Auth::check())
          <li class="nav-item mainnav">
          @if($page=='m')
            <a class="nav-link active" href="{{route('profile.show',['id'=>auth()->user()->id])}}">بنگاه من</a>
          @else
            <a class="nav-link" href="{{route('profile.show',['id'=>auth()->user()->id])}}">بنگاه من</a>
          @endif
          </li>
          <li class="nav-item mainnav">
          @if($page=='f')
