
<div class="container-fluid" style="background-color: rgb(0, 0, 0);">
<footer class="page-footer font-small mdb-color lighten-3 pt-4">
    <div class="container text-center text-md-left">

      <div class="row">
 



        <hr class="clearfix w-100 d-md-none">
        <div class="col-md-12 text-center mx-auto my-4">
          <h5 class="font-weight-bold text-uppercase mb-4" style="color:white;">مارا دنبال کنید</h5>
          <!-- Facebook -->
          <a  class="btn-floating btn-fb" href="#">
              <img src="/pic/1.png" alt="" srcset="">
          </a>
          <!-- Twitter -->
          <a class="btn-floating btn-tw" href="#">
              <img src="/pic/2.png" alt="" srcset="">
          </a>
          <!-- Google +-->
          <a  class="btn-floating btn-gplus" href="#">
            <img src="/pic/3.png" alt="" srcset="">
          </a>
             <!-- insta-->
             <a  class="btn-floating btn-ins" href="#">
            <img src="/pic/4.ico" alt="" srcset="">
          </a>
          <!-- telegram-->
          <a  href="#">
            <img src="/pic/5.ico" alt="" srcset="">
          </a>
          <!-- whatsapp-->
          <a  href="#">
            <img src="/pic/6.ico" alt="" srcset="">
          </a>
        </div>
      </div>
    </div>

    <!-- Copyright -->
    <div class="footer-copyright text-center py-3" style="color:white;">© 2018 Copyright:
      <a> alib327@gmail.com</a>
    </div>
    <!-- Copyright -->

  </footer>
  <!-- Footer -->
   </div> 
